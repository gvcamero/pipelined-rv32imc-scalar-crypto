...
module tb_core_extmem_benchmark();
    ...
    //localparam string temp_inst = $sformatf("%s%s%s", `REPO_LOCATION, `TEST_LOCATION, "instmem-dump/mem/C-BNEZ.mem");
    ...
    //localparam string temp_data = $sformatf("%s%s%s", `REPO_LOCATION, `TEST_LOCATION, "datamem-dump/mem/C-BNEZ.mem");
	...
	always@(posedge CLK) begin
		...
		if (active_data_op && curr_buf_addr > 15'h4FFF && last_buf_addr != curr_buf_addr) begin
		      $write("%c", next_char);
		      buff_char <= next_char;
		end
	end
